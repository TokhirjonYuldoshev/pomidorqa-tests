...
Из продуктовых расхождений аудит требований выявил три заметных случая на live E2E уровне. R5.5 с некорректным применением часового пояса и R11.2 с отсутствующим двухчасовым ограничением отмены были исправлены в продукте и теперь защищены обычными regression tests. R8.3 остаётся открытым: каталог учитывает `want_to_learn`, хотя требование ограничивает поиск навыками `can_help`. Проверка написана по требованию и оформлена как `test.fail()`, поэтому дефект виден в наборе и не маскируется зелёным тестом.
