...
- длительность фиксируется в каждом `CI Summary`, а не считается постоянным числом; контрольный `main` run #341 на commit `531df27` дал Unit **10 passed (939 ms)**, API **11 passed (4.6 s)**, Chromium **100 passed (4.2 min)**, Firefox **100 passed (5.6 min)**, WebKit **100 passed (5.8 min)**;
- run #341 завершился успешно с первого attempt при текущей схеме `workers=4`, `retries=0`, `max-parallel: 2`;
- browser matrix использует `workers=4` внутри job и `max-parallel: 2`, поэтому одновременно работают максимум 8 E2E workers; ограничение появилось после измеренного насыщения live-стенда при 12 workers, а `Regression Gate` агрегирует Quality + Unit + API + E2E matrix перед итоговой сводкой.
- requirement traceability фиксируется до конкретного test case: coverage matrix указывает `test-файл → точное название test(...)`, а валидатор проверяет существование обеих частей ссылки.

Из продуктовых расхождений аудит требований выявил три заметных случая на live E2E уровне. Для R5.5 с некорректным применением часового пояса и R11.2 с нарушением двухчасового ограничения отмены более поздние прогоны показали, что расхождения больше не воспроизводятся; эти сценарии теперь работают как обычные regression tests. Важно не говорить «мы исправили баги»: репозиторий автоматизации не содержит исходный код PomidorQA и позволяет доказать только наблюдаемое поведение black-box стенда, а не конкретную причину или реализацию product fix. R8.3 остаётся открытым: каталог учитывает `want_to_learn`, хотя требование ограничивает поиск навыками `can_help`. Проверка написана по требованию и оформлена как `test.fail()`, поэтому дефект виден в наборе и не маскируется зелёным тестом.

## Как коротко представить проект

> Это инженерный проект по автоматизации тестирования PomidorQA на Playwright и TypeScript. В нём есть Unit, API и E2E уровни, Page Objects, fixtures, подготовка тестовых аккаунтов через API и централизованная очистка данных. E2E проходят в Chromium, Firefox и WebKit с `retries=0`. Отдельно настроены проверки безопасности, доступности, Lighthouse, визуальных изменений, Nightly, Stability, Allure/Playwright отчёты и Telegram-уведомления. Основной акцент — честный тестовый сигнал, изоляция данных и возможность быстро определить причину сбоя.
