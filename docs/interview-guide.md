...
- длительность фиксируется в каждом `CI Summary`, а не считается постоянным числом; контрольный `main` run #296 дал Unit **10 passed (787 ms)**, API **11 passed (3.6 s)**, Chromium **100 passed (4.4 min)**, Firefox **100 passed (5.4 min)**, WebKit **100 passed (6.0 min)**;
- run #296 завершился успешно с первого attempt при текущей схеме `workers=4`, `retries=0`, `max-parallel: 2`;
