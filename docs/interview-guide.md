...
- длительность фиксируется в каждом `CI Summary`, а не считается постоянным числом; контрольный `main` run #294 дал Unit **10 passed (764 ms)**, API **11 passed (5.0 s)**, Chromium **100 passed (5.1 min)**, Firefox **100 passed (6.2 min)**, WebKit **100 passed (5.7 min)**;
- run #294 использует текущую схему `workers=4`, `retries=0`, `max-parallel: 2`; повтор Firefox выполнялся как отдельный GitHub job после единичного transient failure, а не как Playwright retry;
