...
- длительность фиксируется в каждом `CI Summary`, а не считается постоянным числом; контрольный post-merge run #282 завершился успешно с первого attempt, browser jobs заняли примерно Chromium **5:26**, Firefox **6:33**, WebKit **6:42** по wall-clock job;
- run #282 использовал текущую схему `workers=4`, `retries=0`, `max-parallel: 2`; WebKit начал работу после освобождения одного matrix slot, поэтому ограничение нагрузки видно непосредственно в таймингах jobs;
