...
Для каждого требования со статусом, отличным от `out of scope`, evidence фиксируется на уровне **test-файла и точного test case** в формате `tests/...spec.ts` → `название test(...)`. Валидатор проверяет существование и файла, и указанного test case.
