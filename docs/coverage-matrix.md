...
### R5.5 — часовой пояс владельца применялся некорректно → расхождение больше не воспроизводится

На live UI было обнаружено расхождение: владелец создавал слот в своём часовом поясе, но другой пользователь мог видеть смещённое время. Последующие прогоны показывают ожидаемое по требованию поведение, поэтому R5.5 имеет статус `automated` и подтверждается `tests/e2e/slot-timezone.spec.ts`.

### R11.2 — ограничение отмены по времени не соблюдалось → расхождение больше не воспроизводится

На live E2E уровне было обнаружено, что встречу можно отменить слишком близко к началу, несмотря на требование двухчасового ограничения. Последующие прогоны подтверждают, что поздняя отмена отклоняется, поэтому R11.2 подтверждается `tests/e2e/cancel-window.spec.ts`.

Оба случая важны для traceability: автоматизация зафиксировала расхождение с требованием, а более поздние black-box прогоны подтвердили текущее корректное поведение и превратили сценарии в обычную регрессионную защиту. Этот репозиторий не содержит исходный код PomidorQA и не даёт доказательств того, какое именно изменение продукта, конфигурации или окружения устранило наблюдавшееся расхождение; поэтому документация намеренно не атрибутирует product fix.
