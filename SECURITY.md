...
- основной cross-browser CI выполняет E2E с `workers=4` на browser job, `max-parallel: 2` и `retries=0`;
- Nightly выполняет каждый browser job с `workers=1` и `retries=0`;
- Stability по умолчанию использует `workers=1`, повторяет выбранный сценарий через `repeat-each` и сохраняет `retries=0`;
