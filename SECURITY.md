...
- основной cross-browser CI использует `workers=4` на browser job, `max-parallel: 2` и `retries=0`;
- Nightly использует `workers=1` на browser job и `retries=0`;
- Stability по умолчанию использует `workers=1` и всегда сохраняет `retries=0`;
