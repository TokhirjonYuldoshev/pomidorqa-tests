...
const geminiBaseUrl = (
  process.env.GEMINI_API_BASE_URL || DEFAULT_GEMINI_BASE_URL
).replace(/\/$/, "");
...
const ruleNumbers = extractRuleNumbers(codex);

if (!ruleNumbers.length) {
  throw new Error("В CODEX.md не найдены нумерованные правила.");
}
...
async function getPullFiles() {
  ...
  throw new Error(
    `В PR больше ${MAX_FILE_PAGES * 100} файлов — AI-review пропущен.`,
  );
}
...
function prepareDiff(files) {
  ...
  if (withoutPatch.length) {
    throw new Error(
      "GitHub не вернул patch для: " +
        withoutPatch.map((file) => file.filename).join(", "),
    );
  }
  ...
  if (diff.length > MAX_DIFF_CHARS) {
    throw new Error(
      `Diff слишком большой: ${diff.length} символов, ` +
...
function buildReviewPrompts({ pull, diff }) {
  const system = `Ты строгий, но доброжелательный senior QA Automation reviewer проекта PomidorQA на Playwright + TypeScript.
...
Пиши по-русски, конкретно, без эмодзи.`;
  ...
  const user = `МЕТАДАННЫЕ PR (НЕДОВЕРЕННЫЕ ДАННЫЕ)
<pull_request>
Ветка: ${truncate(pull.head.ref, 200)}
Автор: ${truncate(pull.user.login, 100)}
...
async function callGemini({
  ...
  if (!text) {
    const finishReason =
      payload?.candidates?.[0]?.finishReason || "нет";
    const blockReason =
      payload?.promptFeedback?.blockReason || "нет";
...
async function requestReview({
  ...
  const allowedPaths = [...addedLinesByPath.entries()]
    .filter(([, lines]) => lines.size > 0)
    .map(([path]) => path);

  const allowedLines = [
    ...new Set(
      [...addedLinesByPath.values()].flatMap(
        (lines) => [...lines],
      ),
    ),
  ].sort((left, right) => left - right);

  const prompts = buildReviewPrompts({ pull, diff });
  ...
  return {
    review: parseStructuredReview(result.text),
    usage: result.usage,
  };
}
...
async function verifyComments({
  ...
  const system = `Ты второй независимый senior QA reviewer. Твоя задача — защищать автора PR от ложных AI-замечаний.
...
${relevantDiffForComments(diff, comments)}
  ...
  try {
    ...
    throw new Error(
      `Verifier вернул невалидный JSON: ${error.message}`,
    );
  }

  if (!Array.isArray(parsed.checks)) {
    throw new Error(
      "Verifier не вернул массив checks.",
    );
  }

  const returnedIndexes = new Set(
    parsed.checks.map((check) => check.index),
  );

  if (
    returnedIndexes.size !== comments.length ||
    indexes.some(
      (index) => !returnedIndexes.has(index),
    )
  ) {
    throw new Error(
      "Verifier не проверил каждый комментарий ровно один раз.",
    );
  }

  const validIndexes = new Set(
    parsed.checks
      .filter((check) => check.valid === true)
      .map((check) => check.index),
  );

  for (const check of parsed.checks) {
    if (!check.valid) {
      const candidate = comments[check.index];

      console.log(
        `Verifier отклонил: ` +
          `${candidate?.path}:${candidate?.line} — ` +
          `${candidate?.title} — ${check.reason}`,
      );
    }
  }
  ...
  return {
    comments: comments.filter(
      (_, index) => validIndexes.has(index),
    ),
    usage: result.usage,
  };
}
...
async function main() {
  ...
  if (pull.state !== "open" || pull.draft) {
    console.log(
      "PR закрыт или находится в draft — AI-review пропущен.",
    );
    return;
  }
  ...
  if (
    ...
    console.log(
      "PR открыт не в основную ветку — AI-review пропущен.",
    );
    return;
  }
  ...
  if (pull.head.sha !== expectedHeadSha) {
    console.log(
      "После CI в PR появился новый commit — устаревшее review пропущено.",
    );
    return;
  }
  ...
  if (
    ...
    console.log(
      "Этот commit уже проверен AI-reviewer — повторный вызов Gemini не нужен.",
    );
    return;
  }
  ...
  if (!prepared.diff.trim()) {
    console.log(
      "В PR нет изменений, которые входят в область AI-review — пропускаем.",
    );
    return;
  }

  console.log(
    `Gemini model=${model}; diff=${prepared.diff.length} символов.`,
  );
  ...
  const coordinateValid =
    generated.review.comments.filter(
      (comment) =>
        toGitHubComments(
          [comment],
          prepared.addedLinesByPath,
  ...
  const rejectedCoordinates =
    generated.review.comments.length -
    coordinateValid.length;

  if (rejectedCoordinates) {
    console.log(
      `Локальная валидация отбросила комментариев: ${rejectedCoordinates}.`,
    );
  }
  ...
  const freshPull = await githubRequest(
    `/repos/${repository}/pulls/${pullNumber}`,
  );

  if (freshPull.head.sha !== expectedHeadSha) {
    console.log(
      "Во время анализа появился новый commit — результат не опубликован.",
    );
    return;
  }

  const usageText = usage
    ? `Токены: ${usage.promptTokens} вход / ` +
...
Модель: \`${model}\`. ${usageText}`;
  ...
  if (
    ...
    process.env.AI_REVIEW_ALLOW_PUBLISH !==
      "true"
  ) {
    console.log(
      JSON.stringify(
  ...
  console.log(
    `AI-review опубликован: ${published.html_url}`,
  );
}

main().catch((error) => {
  console.error(
    `AI-review не выполнен: ${error.message}`,
  );
  process.exitCode = 1;
});
