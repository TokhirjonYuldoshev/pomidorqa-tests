...
| --- | --- | --- | --- |
| R5.5 — часовой пояс слотов | E2E / live UI | расхождение не воспроизводится | ранее время слота отображалось без корректного применения часового пояса владельца; текущие прогоны подтверждают требование через `slot-timezone.spec.ts` |
| R11.2 — окно отмены | E2E / live UI | расхождение не воспроизводится | ранее отмена допускалась слишком близко к началу встречи; текущие прогоны подтверждают двухчасовое ограничение через `cancel-window.spec.ts` |
...
Эти находки разделяются по типу сигнала: продуктовые расхождения не смешиваются с ограничениями внешнего стенда или CI-инфраструктуры. Для R5.5 и R11.2 документация фиксирует только наблюдаемое black-box поведение: расхождение было воспроизведено ранее и больше не воспроизводится на текущем live-стенде. Репозиторий не содержит исходный код PomidorQA, поэтому причина изменения и конкретный product fix здесь не атрибутируются.
